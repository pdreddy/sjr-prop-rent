...
async function main() {
  ...
  for (const t of tenants) {
    ...
    const unit = existing
      ...
      : await createUnit({ plotNumber: t.plotNumber, tenantName: t.tenantName, moveInDate: t.moveInDate, phoneNumbers, advanceAmount: 0, monthlyRent: t.rent, maintenanceAmount: t.maintenance, active: true });
  ...
  for (const j of newJoiners) {
    ...
    const data = { tenantName: j.tenantName, moveInDate: j.moveInDate, phoneNumbers: j.phone ? [j.phone] : [], advanceAmount: 0, monthlyRent: j.rentSum, maintenanceAmount: 0, active: true };
